...
'        Else
        ...
        sTempPath = GetLongTempPath
        
        Kill gsAppDataPath & "\DoUpdate.exe"
        Kill gsAppDataPath & "\DoUpdate.dat"
        Kill sTempPath & "\DoUpdate.exe"
        Kill sTempPath & "\DoUpdate.dat"
        
        GetPageToFile sOpenUrl, sTempPath & "\DoUpdate.dat"
        
        If FileExists(sTempPath & "\DoUpdate.dat") = False Then
            MsgBox "Das Updatepaket konnte nicht heruntergeladen werden: " & vbCrLf & vbCrLf & sTempPath & "\DoUpdate.dat"
            Exit Sub
        Else
            FileCopy sTempPath & "\DoUpdate.dat", sTempPath & "\DoUpdate.exe"
        End If
        
        If FileExists(sTempPath & "\DoUpdate.exe") = False Then
            MsgBox "Das Updatepaket konnte nicht kopiert werden: " & vbCrLf & vbCrLf & sTempPath & "\DoUpdate.dat  - nach - " & sTempPath & "\DoUpdate.exe"
            Exit Sub
        End If
        ...
        'Icon entfernen
        oFrm.RemoveTrayIcon
        ShellExecute 0, "Open", sTempPath & "\DoUpdate.exe", "/S", "", 1
...
Dim sUrl As String
    ...
    If glbJARVISstate = "--" Then
        gbUseCurl = False
        frmSettings.chkUseCurl.Enabled = False
    End If
